...
float r(int a, float b = 0) { return (static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX)) * a + b; }
...
// A Renderable.
struct Renderable {
  Renderable(float radius, QColor color, QPointF position) : radius(radius), color(color), position(position) {}
...
// UI configuration
struct UIConfig {
  explicit UIConfig(QSize size) : size(size) {}
...
using EntityManager =
    entityx::EntityX<entityx::DefaultStorage, 0, Body, Renderable, Particle, Collideable, Indestructible, UIConfig>;
...
public:
  explicit CursorInputSystem(QQuickView *view, EntityManager &es) : m_view(view) {
    ...
    entity.assign<Body>(QVector2D(0, 0), QVector2D(0, 0));

    entity.assign<Renderable>(radius, QColor(r(128, 127), r(128, 127), r(128, 127), 255), QPointF(radius, radius));
  }
  ...
  Entity get_cursor_entity() { return entity; }
...
public:
  ...
  void update(EntityManager &es, float dt) override {
    ...
    for (int i = 0; i < count - c; i++) {
      ...
      // "Physical" attributes.
      entity.assign<Body>(QVector2D(r(m_size.width()), r(m_size.height())), QVector2D(r(100, -50), r(100, -50)));

      entity.assign<Renderable>(collideable->radius, QColor(r(128, 127), r(128, 127), r(128, 127), 255),
...
public:
  ...
  void update(EntityManager &es, float dt) override {
    ...
    es.for_each<Body>([&](Entity, Body &body) {
      if (body.position.x() + body.direction.x() < 0 || body.position.x() + body.direction.x() >= size.width())
        body.direction.setX(-body.direction.x());
      if (body.position.y() + body.direction.y() < 0 || body.position.y() + body.direction.y() >= size.height())
...
public:
  ...
  void emit_particles(EntityManager &es, Entity entity) {
    ...
    for (int i = 0; i < area; i++) {
      ...
      particle.assign<Body>(body.position + QVector2D(offset * cos(angle), offset * sin(angle)),
                            body.direction + QVector2D(offset * 2 * cos(angle), offset * 2 * sin(angle)), rotationd);
...
private:
  ...
  void collect(EntityManager &es) {
    es.for_each<Body, Collideable>([&](Entity entity, Body &body, Collideable &collideable) {
      const unsigned int left = static_cast<int>(body.position.x() - collideable.radius) / PARTITIONS,
      ...
      if (slts[0] != slts[1])
        grid[slts[1]].push_back(candidate);
      if (slts[1] != slts[2])
        grid[slts[2]].push_back(candidate);
      if (slts[2] != slts[3])
        grid[slts[3]].push_back(candidate);
    });
  }
...
public:
  explicit ParticleRenderSystem(QQuickView *view)
      : m_view(view), m_particleComponent(new QQmlComponent(m_view->engine(), QUrl("qrc:/Particle.qml"))) {}
...
public:
  explicit RenderSystem(QQuickView *view) : m_view(view) {
...
public:
  explicit Engine(QQuickView *view, bool benchmark = false) : QObject(view), m_cfg(m_entities.create()), m_view(view) {
  ...
  EntityManager *em() { return &m_entities; }
  ...
  float realFps() const { return m_realFps; }

public slots:
  void step(float stepSize) { update(stepSize); }
...
int main(int argc, char **argv) {
  ...
  QObject::connect(&view, &QQuickView::widthChanged,
                   [&engine](int width) { engine.cfgEntity().component<UIConfig>()->size.setWidth(width); });
  QObject::connect(&view, &QQuickView::heightChanged,
                   [&engine](int height) { engine.cfgEntity().component<UIConfig>()->size.setHeight(height); });
  ...
  QObject::connect(&engine, &Engine::fpsChanged,
                   [&engine, &engineTimer](int fps) { engineTimer.setInterval(1000. / engine.fps()); });

  QObject::connect(&engineTimer, &QTimer::timeout, [&engine, &engineTimer]() { engine.update(1. / engine.fps()); });
