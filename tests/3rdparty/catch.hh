...
#define INTERNAL_CATCH_TESTCASE(...)                                                                                   \
  ...
  namespace {                                                                                                          \
  Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(&INTERNAL_CATCH_UNIQUE_NAME(____C_A_T_C_H____T_E_S_T____),  \
...
#define INTERNAL_CATCH_TEST_CASE_METHOD(ClassName, ...)                                                                \
  ...
  Catch::AutoReg                                                                                                       \
      INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(&INTERNAL_CATCH_UNIQUE_NAME(____C_A_T_C_H____T_E_S_T____)::test,       \
...
public:
  ...
  operator T() const {
    ...
    return T(); // Suppress spurious "not all control paths return a value" warning in Visual Studio - if you know how
                // to fix this please do so
  }
...
struct Contains : MatcherImpl<Contains, std::string> {
  ...
  virtual bool match(std::string const &expr) const { return expr.find(m_substr) != std::string::npos; }
...
struct EndsWith : MatcherImpl<EndsWith, std::string> {
  ...
  virtual bool match(std::string const &expr) const { return expr.find(m_substr) == expr.size() - m_substr.size(); }
...
inline Impl::StdString::Equals Equals(std::string const &str) { return Impl::StdString::Equals(str); }
inline Impl::StdString::Equals Equals(const char *str) {
  return Impl::StdString::Equals(Impl::StdString::makeString(str));
}
inline Impl::StdString::Contains Contains(std::string const &substr) { return Impl::StdString::Contains(substr); }
...
struct ConfigData {
  ...
  ConfigData()
      : listTests(false), listTags(false), listReporters(false), listTestNamesOnly(false), showSuccessfulTests(false),
        shouldDebugBreak(false), noThrow(false), showHelp(false), showInvisibles(false), abortAfter(-1),
        verbosity(Verbosity::Normal), warnings(WarnAbout::Nothing), showDurations(ShowDurations::DefaultForReporter) {}
...
public:
  ...
  void setStreamBuf(std::streambuf *buf) { m_os.rdbuf(buf ? buf : std::cout.rdbuf()); }
...
inline std::size_t listTests(Config const &config) {
  ...
  for (std::vector<TestCase>::const_iterator it = matchedTestCases.begin(), itEnd = matchedTestCases.end(); it != itEnd;
       ++it) {
    matchedTests++;
...
inline std::size_t listReporters(Config const & /*config*/) {
  ...
  for (it = itBegin; it != itEnd; ++it) {
    ...
    std::cout << "  " << it->first << ":" << std::string(maxNameLen - it->first.size() + 2, ' ') << wrapper << "\n";
  }
  std::cout << std::endl;
...
public:
  explicit RunContext(Ptr<IConfig const> const &config, Ptr<IStreamingReporter> const &reporter)
      : m_runInfo(config->name()), m_context(getCurrentMutableContext()), m_activeTestCase(NULL), m_config(config),
        m_reporter(reporter), m_prevRunner(m_context.getRunner()), m_prevResultCapture(m_context.getResultCapture()),
...
private:
  void runCurrentTest(std::string &redirectedCout, std::string &redirectedCerr) {
    ...
    try {
      ...
      ResultBuilder exResult(m_lastAssertionInfo.macroName.c_str(), m_lastAssertionInfo.lineInfo,
...
struct Version {
  Version(unsigned int _majorVersion, unsigned int _minorVersion, unsigned int _buildNumber,
          char const *const _branchName)
      : majorVersion(_majorVersion), minorVersion(_minorVersion), buildNumber(_buildNumber), branchName(_branchName) {}
...
public:
  ...
  virtual void getFilteredTests(TestSpec const &testSpec, IConfig const &config,
    ...
    for (std::vector<TestCase>::const_iterator it = m_functionsInOrder.begin(), itEnd = m_functionsInOrder.end();
         it != itEnd; ++it) {
...
public: // IRegistryHub
  ...
  virtual IExceptionTranslatorRegistry &getExceptionTranslatorRegistry() { return m_exceptionTranslatorRegistry; }

public: // IMutableRegistryHub
  virtual void registerReporter(std::string const &name, IReporterFactory *factory) {
    m_reporterRegistry.registerReporter(name, factory);
  }
  virtual void registerTest(TestCase const &testInfo) { m_testCaseRegistry.registerTest(testInfo); }
...
private:
  void setTextAttribute(WORD _textAttribute) { SetConsoleTextAttribute(stdoutHandle, _textAttribute); }
  HANDLE stdoutHandle;
...
static bool shouldUseColour() { return shouldUseColourForPlatform() && !isDebuggerActive(); }
} // namespace
...
Detail::IColourImpl *Colour::impl() { return shouldUseColour() ? platformColourInstance() : NoColourImpl::instance(); }
...
TestCaseInfo::TestCaseInfo(std::string const &_name, std::string const &_className, std::string const &_description,
                           std::set<std::string> const &_tags, SourceLineInfo const &_lineInfo)
    : name(_name), className(_className), description(_description), tags(_tags), lineInfo(_lineInfo),
      properties(None) {
  std::ostringstream oss;
...
unsigned int Timer::getElapsedNanoseconds() const { return static_cast<unsigned int>(getCurrentTicks() - m_ticks); }
unsigned int Timer::getElapsedMilliseconds() const {
  return static_cast<unsigned int>((getCurrentTicks() - m_ticks) / 1000);
}
double Timer::getElapsedSeconds() const { return (getCurrentTicks() - m_ticks) / 1000000.0; }
...
std::string toString(unsigned int value) { return toString(static_cast<unsigned long>(value)); }
...
ResultBuilder::ResultBuilder(char const *macroName, SourceLineInfo const &lineInfo, char const *capturedExpression,
                             ResultDisposition::Flags resultDisposition)
    : m_assertionInfo(macroName, lineInfo, capturedExpression, resultDisposition), m_shouldDebugBreak(false),
      m_shouldThrow(false) {}
...
struct StreamingReporterBase : SharedImpl<IStreamingReporter> {
  ...
  virtual void testRunStarting(TestRunInfo const &_testRunInfo) { currentTestRunInfo = _testRunInfo; }
  ...
  virtual void sectionStarting(SectionInfo const &_sectionInfo) { m_sectionStack.push_back(_sectionInfo); }
  ...
  virtual void testGroupEnded(TestGroupStats const & /* _testGroupStats */) { currentGroupInfo.reset(); }
...
public:
  ...
  void writeAssertion(AssertionStats const &stats) {
    ...
    if (!result.isOk()) {
      ...
      for (std::vector<MessageInfo>::const_iterator it = stats.infoMessages.begin(), itEnd = stats.infoMessages.end();
           it != itEnd; ++it)
        if (it->type == ResultWas::Info)
          oss << it->message << "\n";
