...
template <typename T, typename Tail, typename... Ts>
struct get_component_size<T, Tail, Ts...> : std::integral_constant<std::size_t, get_component_size<T, Ts...>::value> {};
...
public:
  ...
  ContiguousStorage() { resize(CHUNK_SIZE * INITIAL_CHUNKS); }
  ContiguousStorage(const ContiguousStorage &) = delete;
  ~ContiguousStorage() { reset(); }
  ...
  C *get(std::uint32_t entity) {
    ...
    return static_cast<C *>(static_cast<void *>(blocks_[entity / CHUNK_SIZE] +
...
public:
  ...
  public:
     ...
     */
    bool valid() const { return manager_ && manager_->valid(id_); }
    ...
    ComponentMask component_mask() const { return manager_->component_mask(id_); }
  ...
  public:
    ...
    public:
      Iterator(EntityX *manager, const ComponentMask &mask, uint32_t index)
  ...
  private:
    ...
    BaseView(EntityX *manager, ComponentMask mask) : manager_(manager), mask_(mask) {}
   ...
   */
  AllView all_entities() { return AllView(this); }
...
private:
  ...
  typename std::enable_if<FeatureMask & OBSERVABLE>::type entity_created_(Entity entity) {
    if (on_entity_created_)
      on_entity_created_(entity);
  }
  template <bool FeatureMask = Features>
  typename std::enable_if<!(FeatureMask & OBSERVABLE)>::type entity_created_(Entity) {}

  template <bool FeatureMask = Features>
  typename std::enable_if<FeatureMask & OBSERVABLE>::type entity_destroyed_(Entity entity) {
    if (on_entity_destroyed_)
      on_entity_destroyed_(entity);
  }
  template <bool FeatureMask = Features>
  typename std::enable_if<!(FeatureMask & OBSERVABLE)>::type entity_destroyed_(Entity) {}

  template <class C, bool FeatureMask = Features>
  typename std::enable_if<FeatureMask & OBSERVABLE>::type component_added_(Entity entity, C *component) {
  ...
  template <class C, bool FeatureMask = Features>
  typename std::enable_if<!(FeatureMask & OBSERVABLE)>::type component_added_(Entity, C *) {}

  template <class C, bool FeatureMask = Features>
  typename std::enable_if<FeatureMask & OBSERVABLE>::type component_removed_(Entity entity, C *component) {
  ...
  template <class C, bool FeatureMask = Features>
  typename std::enable_if<!(FeatureMask & OBSERVABLE)>::type component_removed_(Entity, C *) {}
   ...
   */
  Id create_id(uint32_t index) const { return Id(index, entity_version_[index]); }
  ...
  std::function<void(Entity, void *)> on_component_added_[component_count], on_component_removed_[component_count];
};
...
template <template <typename... Cs> class StorageType, std::size_t Features, class... Cs>
inline typename EntityX<StorageType, Features, Cs...>::Entity EntityX<StorageType, Features, Cs...>::create() {
